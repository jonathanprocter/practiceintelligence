#!/usr/bin/env python3
"""
PyMyPDF Bidirectional Export - COMPLETE IMPLEMENTATION
=====================================================

This script creates a bidirectionally linked 8-page PDF using PyMuPDF with proper
clickable navigation between weekly overview and daily pages.
...
def create_weekly_overview_page(page: fitz.Page, events: List[Dict], week_start: datetime):
    ...
    for i, day_name in enumerate(days):
        ...
        # Day header - make it blue to indicate it's clickable
        ...
        page.insert_text((x + 5, grid_start_y + 20), f"{day_name}", fontsize=10, color=(0, 0, 1))  # Blue for clickable
...
def create_daily_page(page: fitz.Page, events: List[Dict], date: datetime, day_name: str, day_index: int):
    ...
    # Navigation buttons area (clickable links)
    nav_y = 100
    page.draw_rect(fitz.Rect(50, nav_y, 150, nav_y + 25), color=(0.9, 0.9, 0.9), width=1)
    page.insert_text((55, nav_y + 15), "📅 Weekly Overview", fontsize=10, color=(0, 0, 1))  # Blue for clickable

    # Previous/Next day navigation
    if day_index > 0:  # Not Monday
        page.draw_rect(fitz.Rect(200, nav_y, 280, nav_y + 25), color=(0.9, 0.9, 0.9), width=1)
        page.insert_text((205, nav_y + 15), f"← {['Mon','Tue','Wed','Thu','Fri','Sat','Sun'][day_index-1]}", fontsize=10, color=(0, 0, 1))

    if day_index < 6:  # Not Sunday
        page.draw_rect(fitz.Rect(320, nav_y, 400, nav_y + 25), color=(0.9, 0.9, 0.9), width=1)
        page.insert_text((325, nav_y + 15), f"{['Mon','Tue','Wed','Thu','Fri','Sat','Sun'][day_index+1]} →", fontsize=10, color=(0, 0, 1))
